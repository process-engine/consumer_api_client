...
function setPackageVersion(version) {
  const csProj = fs.readFileSync('./src/ProcessEngine.ConsumerAPI.Client.csproj', {encoding: 'utf8'});
  const csProjWithNewVersion = csProj.replace(getPackageVersion().toString(), version);
  fs.writeFileSync('./src/ProcessEngine.ConsumerAPI.Client.csproj', csProjWithNewVersion);
